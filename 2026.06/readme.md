# Event Details
* Event: ONNX Community Meetup 2026 
* Date: June 26, 2026 (Friday) 
* Location: Microsoft Silicon Valley Campus (1045 La Avenida St, Mountain View, CA)
* Format: In-person event with livestream access 

# Technical Talks
* Mobius - Agentic model conversion for ONNX (Justin Chu)
* DiTOX: Fault Detection and Localization in the ONNX Optimizer (Nikolaos Louloudakis) 
* ONNX and the Future of Verifiable AI (Sean Westfall) 
* Lessons Learned Building an ONNX-to-C Compiler with 100% Test Coverage (Timo Stripf) 
* Neural Network Compression Framework for ONNX (Freddy Chiu & Sahar Fatima) 
* Beyond Latency: Understanding ONNX Execution on AI PCs (Freddy Chiu & Vasanth Tovinkere)
* ONNX: Next Steps (Christian Bourjau)

# Other Agenda Items: TBD

 9:00 -  9:10: Welcome
 9:10 -  9:30: DiTOX: Fault Detection and Localization in the ONNX Optimizer (Nikolaos Louloudakis) 
 9:30 - 10:00: SONNX and AIEDGE (Eric Jenn, Jean Souyris, Pierre Gaillard)
10:00 - 10:20: Lessons Learned Building an ONNX-to-C Compiler with 100% Test Coverage (Timo Stripf)
10:20 - 10:30: Neural Network Compression Framework for ONNX (Freddy Chiu & Sahar Fatima) 

10:30 - 11:00: Break

11:00 - 11:10: Arch/Infra SIG Update (Andreas Fehlner & Christian Bourjau)
11:10 - 11:20: ONNX: Next Steps (Christian Bourjau)
11:20 - 11:40: Beyond Latency: Understanding ONNX Execution on AI PCs (Freddy Chiu & Vasanth Tovinkere)
11:40 - 12:00: Mobius - Agentic model conversion for ONNX (Justin Chu)

12:00 - 01:30: lunch

01:30 - 01:50: ONNX and the Future of Verifiable AI (Sean Westfall)
01:50 - 02:00: GenAI WG Update (Yamini Nimmagadda, Javier Martinez, G. Ramalingam) 
02:00 - 02:10: Compiler SIG Update (Alexandre Eichenberger)
02:10 - 02:20: Converter SIG Update (Xavier Dupre) 
02:20 - 02:30: Probabilistic Programming WG Update (Brian Parbhu, Adam Pocock)



